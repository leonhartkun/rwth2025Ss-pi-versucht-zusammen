...
// Vorwürtsdeklarationen aller vorhandenen Funktionen
...
int main()
{
    ...
    // Beim Beenden des Programms sümtliche Medien lüschen
...
/////////////////////////////////////////////////////////////
// Definition aller Vorwürtsdeklarierten Funktionen
...
void fuelleDatenbank(std::vector<Medium*>& medien)
{
#if UNTERKLASSENVORHANDEN
    {
        ...
        neuesMedium = new DVD("Fluch der Karibik", 12, "Actionkomüdie");
        medien.push_back(neuesMedium);
...
void mediumHinzufuegen(std::vector<Medium*>& medien)
{
    #if UNTERKLASSENVORHANDEN
    {
        ...
        // über std::cin.ignore(10,\n) wird die "Enter-Eingabe, die für std::cin benütigt wird ignoriert
        // Ansonsten würde der nüchste getline Befehl nicht auf eine Eingabe warten...
        char abfrage;
        std::cin >> abfrage;
        std::cin.ignore(10,'\n');
...
void mediumEntfernen(std::vector<Medium*>& medien)
{
    // ID des Mediums, welches gelüscht werden soll

    unsigned int id;

    std::cout << "Geben Sie die ID des Mediums ein, welches gelüscht werden soll: " ;
    std::cin >> id;
    std::cin.ignore(10, '\n');

    bool success = false;

    // Suchen des richtigen Mediums zum Lüschen
    for(std::vector<Medium*>::iterator it = medien.begin(); it != medien.end(); it++)
    {
        if((*it)->getID() == id)
        {
            delete (*it);  // Objekt lüschen
            medien.erase(it); // Zeiger auf das Objekt lüschen
            success = true; // Lüschen war erfolgreich
...
void mediumAusleihen(std::vector<Medium*>& medien, Datum aktuellesDatum)
{
    ...
    // Einlesen des Namens der Person, die das Medium ausleihen müchte
    std::string name;
